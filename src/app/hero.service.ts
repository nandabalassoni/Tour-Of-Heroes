...
const httpOptions = {
  headers: new HttpHeaders({ 'Content-Type': 'application/json' }) // cabeçalho especial nas solicitações de salvamento HTTP.
};
...
export class HeroService {
  ...
  /* Os HeroServicemétodos acessarão o fluxo de valores observáveis ​​e enviarão uma mensagem (via log()) para a área de mensagem na parte inferior da página.

Eles farão isso com o tapoperador RxJS , que olha os valores observáveis, faz algo com esses valores e os repassa. A tapchamada de volta não afeta os próprios valores. */
  ...
  /* O método retorna imediatamente com uma matriz vazia se não houver um termo de pesquisa. O resto se assemelha muito getHeroes(). A única diferença significativa é o URL, que inclui uma string de consulta com o termo de pesquisa. */
